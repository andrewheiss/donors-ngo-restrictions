...
```{r load-dcjw, warning=FALSE, message=FALSE}
# Original DCJW data
...
dcjw.questions.raw <- read_csv(file.path(PROJHOME, "Data", "data_clean",
                                         "dcjw_questions.csv"))
...
dcjw.panel.barriers <- dcjw.panel.all.laws %>%
  ...
  mutate(association.std = association / 2,
         entry.std = entry / 3,  # 4, but 1 ignored
         funding.std = funding / 7,  # 8, but 1 ignored
...
- `entry` (Q2b, Q2c, Q2d; 3 points maximum): barriers to entry
- Q2c is reversed, so *not* being allowed to appeal registration status earns 1 point.
- Q2a is omitted because it's benign
- `funding` (Q3b, Q3c, Q3d, Q3e, Q3f; 5 points maximum): barriers to funding
- Q3s id omitted because it's benign
- Scores that range between 0–2 are rescaled to 0–1 (so 1 becomes 1.5)
- `advocacy` (Q4a, Q4b, Q4c; 3 points maximum): barriers to advocacy
- Scores that range between 0–2 are rescaled to 0–1 (so 1 becomes 1.5)
- `barriers.total` (13 points maximum, though actual max is `r max(dcjw.full$barriers.total)`): sum of all four indexes
...
wdi.path <- file.path(PROJHOME, "data", "data_cache", "wdi.rds")
...
imputation.rds <- file.path(PROJHOME, "Data", "data_cache",
                            "country_aid_data_imputed_long.rds")

if (file.exists(imputation.rds)) {
  country.aid.data.imputed.long <- readRDS(file.path(PROJHOME, "Data", "data_cache",
                                                     "country_aid_data_imputed_long.rds"))
} else {
  # THIS TAKES SO LONG.
  #
  ...
  country.aid.data.imputed.long <- 
    bind_rows(mutate(country.aid.data.to.impute, m = "original"),
              bind_rows(country.aid.data.amelia$imputations, .id = "m")) %>%
    select(m, everything()) %>%
    left_join(select(country.aid.data.pre.amelia, 
                     -one_of(vars.to.include.sans.ids)), by = c("cowcode", "year"))
  
  saveRDS(country.aid.data.imputed.long, file.path(PROJHOME, "Data", "data_cache",
                                                   "country_aid_data_imputed_long.rds"))
}
```
...
saveRDS(donor.level.data.usaid,
        file = file.path(PROJHOME, "Data", "data_clean",
                         "df_donor_usaid.rds"))
```
