...
aiddata.final <- aiddata.clean %>%
  ...
  select(donor, donor.type, donor.cowcode, donor.iso3, year,
         ...
         oda.total = commitment_amount_usd_constant_sum,
         purpose.code.short, purpose.sector, purpose.category,
...
visdat::vis_miss(dcjw.full) + theme(legend.position="top")
```
...
vdem.cso.min %>% datatable(extensions="Responsive")
```
...
wdi.clean %>% datatable(extensions="Responsive")
...
disasters.groups %>% datatable(extensions="Responsive")
```
...
donor.type.total <- donor.level.data %>%
  group_by(cowcode, donor.type, year) %>%
  summarise(total.aid = sum(oda.total, na.rm=TRUE)) %>%
  ungroup() %>%
  mutate(donor.type = recode(donor.type, Country = "aid.country", 
                             `Multilateral or IGO` = "aid.igo",
                             `Private donor` = "aid.private")) %>%
  spread(donor.type, total.aid) %>%
  # Make a full panel and fill with 0s
  right_join(expand(., cowcode, year), by=c("cowcode", "year")) %>%
  mutate_at(vars(starts_with("aid")), funs(ifelse(is.na(.), 0, .))) %>%
  mutate(aid.total = aid.country + aid.igo + aid.private) %>%
  mutate_at(vars(starts_with("aid")), funs(log = log1p(.)))

donor.purpose.total <- donor.level.data %>%
  group_by(cowcode, purpose.contentiousness, year) %>%
  summarise(total.aid = sum(oda.total, na.rm=TRUE)) %>%
  ungroup() %>%
  mutate(purpose.contentiousness = recode(purpose.contentiousness,
                                          High = "contention.high",
                                          Low = "contention.low")) %>%
  spread(purpose.contentiousness, total.aid) %>%
  mutate_at(vars(starts_with("contention")), funs(ifelse(is.na(.), 0, .))) %>%
  mutate_at(vars(starts_with("contention")), funs(log = log1p(.)))
...
country.level.data <- dcjw.full %>%
  ...
  left_join(donor.type.total, by=c("cowcode", "year")) %>%
