...
- **Government capacity**: donors likely give more aid to NGOs in countries with weak institutions (and perhaps less aid overall?)
    - Corruption: `corruption` (0-1 scale from V-Dem: public sector + executive + + legislative + judicial corruption, originally `v2x_corr`)
    - Quality of governance: `govt.effectiveness` (≈-2 to 2 scale from V-Dem and WBGI, originally `e_wbgi_gee`)
...
aid.by.country.purpose <- donor.country.data %>%
  ...
  mutate(prop.contentious = High / (Low + High)) %>%
...
# Add aid and diffed + leaded data to main panel
country.aid.data <- country.level.data %>%
  ...
  # Lag/lead/diff everything within countries
  group_by(cowcode) %>%
  ...
  mutate_at(vars(contains("oda"), -contains("log")),
...
This data is relatively complete, with only a few variables suffering from missing data, mostly from the World Bank (and some from V-Dem).

```{r final-data-missing-donors, fig.width=6, fig.height=4.5}
plot.percent.missing(donor.country.data)
```

There are more missing variables in the country-level panel data, but that's because of shifting windows—when variables are leaded or lagged, the first or last observation is missing

```{r final-data-missing-countries, fig.width=6, fig.height=5}
plot.percent.missing(country.aid.data)
```
...
saveRDS(donor.country.data,
        file=file.path(PROJHOME, "Data", "data_clean",
                       "df_donor_country.rds"))

saveRDS(country.level.data,
        file=file.path(PROJHOME, "Data", "data_clean",
                       "df_country.rds"))

saveRDS(country.aid.data,
        file=file.path(PROJHOME, "Data", "data_clean",
                       "df_country_aid.rds"))
