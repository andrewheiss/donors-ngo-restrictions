...
ever.dac.eligible <- read_csv(file.path(PROJHOME, "Data", "data_clean", 
...
dcjw.barriers.clean <- dcjw.questions.clean %>%
...
if (all.na.means.zero) {
  dcjw.panel.all.laws <- dcjw.panel.all.laws %>%
    ...
    mutate(value.fixed = case_when(
      is.na(value) & .$question == "q_1a" ~ as.integer(0),
      is.na(value) & .$question == "q_1b" ~ as.integer(0),
      is.na(value) & .$question == "q_2a" ~ as.integer(0),
      is.na(value) & .$question == "q_2b" ~ as.integer(0),
      is.na(value) & .$question == "q_2c" ~ as.integer(0),
      is.na(value) & .$question == "q_2d" ~ as.integer(0),
      is.na(value) & .$question == "q_3a" ~ as.integer(0),
      is.na(value) & .$question == "q_3b" ~ as.integer(0),
      is.na(value) & .$question == "q_3c" ~ as.integer(0),
      is.na(value) & .$question == "q_3d" ~ as.integer(0),
      is.na(value) & .$question == "q_3e" ~ as.integer(0),
      is.na(value) & .$question == "q_3f" ~ as.integer(0),
      is.na(value) & .$question == "q_4a" ~ as.integer(0),
      is.na(value) & .$question == "q_4b" ~ as.integer(0),
      is.na(value) & .$question == "q_4c" ~ as.integer(0),
      TRUE ~ .$value
    )) %>%
    select(-value) %>%
...
```{r load-un}
# UN country codes
un.codes <- read_csv(file.path(PROJHOME, "Data",
                               "data_clean", "un_codes.csv")) %>%
  select(un_code, iso3)
...
un.gdp.raw <- read_csv(file.path(PROJHOME, "Data", "data_raw", "UN data",
...
un.gdp.current.raw <- read_csv(file.path(PROJHOME, "Data", "data_raw", "UN data",
                                 "UNdata_Export_20170125_063718499.csv")) %>%
  rename(un_code = `Country or Area Code`) %>%
  mutate(value.type = "Current")

# https://esa.un.org/unpd/wpp/Download/Standard/Population/
un.pop.raw <- read_excel(file.path(PROJHOME, "Data", "data_raw", "UN data",
...
un.gdp.final %>% head(100) %>% datatable(extensions="Responsive")

un.gdp.final %>% glimpse()
```
...
#                             ordered=TRUE))
conflicts.raw <- read_csv("http://ucdp.uu.se/downloads/ucdpprio/ucdp-prio-acd-4-2016.csv")
...
country.level.data <- dcjw.full %>%
  ...
  # Add indicator for post-Cold War, but no real need to use it since we
  # account for time in the random effects and in the current year's aid. The
  # post-Cold War coefficients are all hugely significant, but that's mostly
  # because there's more aid given after the Cold War anyway and it doesn't
  # really help explain much.
...
plot.percent.missing(donor.level.data)
```
...
```{r final-data-missing-countries, fig.width=6, fig.height=5}
plot.percent.missing(country.level.data)
```

That said, it's still helpful to impute the missing variables, especially since polity, corruption, and GDP are all main controls (plus [Lall 2016](https://doi.org/10.1093/pan/mpw020) shows the downsides of not imputing). So, we use [Amelia](http://gking.harvard.edu/amelia) to impute missing variables.
...
```{r stacked-data-example, results="asis"}
stacked.data <- tribble(
...
pandoc.table(stacked.data)
```

We estimate models for each of the imputed datasets and recombine their results. We use `amelia::mi.meld()` to combine estimates and standard errors from frequentist models (following [King et al. 2001](http://gking.harvard.edu/files/abs/evil-abs.shtml)), and we [merge the MCMC chains](https://groups.google.com/d/msg/stan-users/daqP88cFQGY/F-gJLkFuXlQJ) from Bayesian models ([see also this](https://groups.google.com/forum/#!topic/stan-users/0hqV_RDx9MU)).

We varied the number of datasets we imputed (*m* = 5 and *m* = 10) and [found only trivial differences in coefficients between 5 and 10 datasets](../Analysis/models.html#imputation) (as predicted by King et al.). In the interest of computational efficiency and speed, we only use 5 imputed datasets. To avoid needing to recreate both 5 and 10 datasets, we impute 10, but only use the first 5 for analysis.
...
vars.to.ignore.in.imputation <- country.aid.data.pre.transformation %>%
  select(year, cowcode,
         which(sapply(.,is.character)),
         const_assoc, political_parties, starts_with("ngo"), contains("std"),
         starts_with("barriers"), contains("contentious"),
         natural_disaster.total.affected, col.ind.date, post.1989)

# Vector of variables to exclude
vars.to.ignore.sans.ids <- vars.to.ignore.in.imputation %>%
  select(-year, -cowcode) %>% colnames()

# Stripped down dataset for imputation
country.aid.data.to.impute <- country.aid.data.pre.transformation %>%
  select(-one_of(vars.to.ignore.sans.ids))
...
set.seed(my.seed)
country.aid.data.imputed <- amelia(country.aid.data.to.impute,
                                   m=10, ts="year", cs="cowcode", p2s=0,
...
country.aid.data.imputed.long <- 
  bind_rows(mutate(country.aid.data.to.impute, m="original"),
            bind_rows(country.aid.data.imputed$imputations, .id="m")) %>%
  select(m, everything()) %>%
  left_join(vars.to.ignore.in.imputation, by=c("cowcode", "year"))
```

```{r log-lead-lag-everything}
...
country.aid.data <- country.aid.data.imputed.long %>%
  ...
  mutate(prop.contentious = ifelse(is.nan(prop.contentious), 
                                   NA, prop.contentious)) %>%
  mutate_at(vars(total.oda, oda.contentious.high, oda.contentious.low),
            funs(log = log1p(.))) %>%
  mutate(aid.pct.gdp = total.oda / gdp) %>%
  # Lag/lead/diff everything within countries and samples
  group_by(m, cowcode) %>%
  # Get leaded aid variables
  mutate_at(vars(contains("oda"), prop.contentious),
...
country.aid.data %>% head(100) %>% datatable(extensions="Responsive")

country.aid.data %>% glimpse()
```
...
```{r plot-missing-imputed, fig.width=8, fig.height=5}
plot.percent.missing(filter(country.aid.data, m != "original"), 3)
```
...
saveRDS(filter(country.aid.data, m == "original"),
        file=file.path(PROJHOME, "Data", "data_clean",
...
saveRDS(filter(country.aid.data.m10, m != "original"),
        file=file.path(PROJHOME, "Data", "data_clean",
                       "df_country_aid_imputation_m10.rds"))

saveRDS(donor.level.data,
        file=file.path(PROJHOME, "Data", "data_clean",
                       "df_donor.rds"))
```
