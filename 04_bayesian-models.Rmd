...
```{r h2-predict-both, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r h2-predict-both-plot, fig.width=5.5, fig.height=2.5}
df.plot.data.h2 <- bind_rows(df.plot.h2.advocacy, df.plot.h2.csre)
...
```{r h3-predict-both, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r h3-predict-both-plot, fig.width=5.5, fig.height=2.5}
df.plot.data.h3 <- bind_rows(df.plot.h3.foreign, df.plot.h3.domestic)
...
fig.predicted.h3.barriers <- ggplot(df.plot.data.h3, 
                                    aes(x = variable, 
                                        y = value.fixed, group = cowcode)) + 
  geom_vline(xintercept = 0, colour = "black", size = 0.5) + 
  geom_smooth(method = "lm", size = 0.1, alpha = 0.1, colour = simulation.individual) +
  geom_smooth(data = df.plot.data.mean.h3, size = 1.5, method = "lm", colour = simulation.mean) +
  labs(x = "Difference from average number of barriers to funding (within effect)",
