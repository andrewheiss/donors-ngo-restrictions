...
```{r h1-models-bayes-1, warning=FALSE, message=FALSE, cache=TRUE}
# Get scaling factors
...
mods.h1.next_year.bayes <- mods.h1.next_year.raw.bayes %>%
  ...
  mutate(glance = model %>% map(glance),
         tidy = model %>% map(tidy)) %>% 
  bind_cols(select(scales.h1.next_year, -m))
```
...
```{r h1-diag-density}
pp_check(model.to.check, type = "dens_overlay", nsamples = 10) + 
  theme_donors()
```
...
rstan::stan_diag(model.to.check$fit, information = "divergence")
```
...
```{r h1-plot-coefs, fig.width=5.5, fig.height=2.5}
...
Another solution is to use [beta regression](https://cran.r-project.org/package=betareg), which [constrains the outcome variable to values between 0 and 1](http://www.theanalysisfactor.com/zero-one-inflated-beta-models-for-proportion-data/), but unfortunately does not allow for values of exactly 0 or 1. Zero-and-one inflated beta regression models, however, make adjustments for this, and we include a set of these models as a robustness check. We stick with logit-linear models, though, since we want to compare the ratio of contentious aid to non-contentious aid (rather than the percent of contentious aid) and keep the comparison parallel with our H3 models, which only compare the ratio of aid channeled through all NGOs.
...
```{r h2-models-bayes, warning=FALSE, message=FALSE, cache=TRUE}
# Get scaling factors
...
mods.h2.next_year.bayes <- mods.h2.next_year.raw.bayes %>%
  ...
  mutate(glance = model %>% map(glance),
         tidy = model %>% map(tidy)) %>% 
  bind_cols(select(scales.h2.next_year, -m))
```
...
```{r h2-plot-coefs, fig.width=5.5, fig.height=2.5}
vars.to.plot <- c("barriers.total_within", "barriers.total_between",
                  "advocacy_within", "advocacy_between", 
                  "entry_within", "entry_between", 
                  "funding_within", "funding_between",
...
```{r h3-models-bayes-domestic, warning=FALSE, message=FALSE, cache=TRUE}
# Get scaling factors
...
mods.h3.dom.next_year.bayes <- mods.h3.dom.next_year.raw %>%
  ...
  mutate(glance = model %>% map(glance),
         tidy = model %>% map(tidy)) %>% 
  bind_cols(select(scales.h3.dom.next_year, -m))
```

```{r h3-models-bayes-foreign, warning=FALSE, message=FALSE, cache=TRUE}
# Get scaling factors
...
mods.h3.foreign.next_year.bayes <- mods.h3.foreign.next_year.raw %>%
  ...
  mutate(glance = model %>% map(glance),
         tidy = model %>% map(tidy)) %>% 
  bind_cols(select(scales.h3.foreign.next_year, -m))
```
...
```{r plot-h3-coefs-both, warning=FALSE, message=FALSE, fig.width=5.5, fig.height=3}
vars.to.plot <- c("barriers.total_within", "barriers.total_between",
                  "advocacy_within", "advocacy_between", 
                  "entry_within", "entry_between", 
                  "funding_within", "funding_between",
...
predictions <- tribble(
  ...
  walk2(.x = .$prediction, .y = .$filename, 
        .f = ~ write_csv(.x, path = here("Data", "data_export", paste0(.y, ".csv"))))
```
...
```{r h2-models-bayes-zoib, warning=FALSE, message=FALSE, cache=TRUE}
# Get scaling factors
...
mods.h2.next_year.bayes.zoib <- mods.h2.next_year.raw.bayes.zoib %>%
  ...
  mutate(glance = model %>% map(glance),
         tidy = model %>% map(tidy)) %>% 
  bind_cols(select(scales.h2.next_year.zoib, -m))
```
...
```{r h2-plot-coefs-zoib, fig.width=5.5, fig.height=2.5}
vars.to.plot <- c("barriers.total_within", "barriers.total_between",
                  "advocacy_within", "advocacy_between", 
                  "entry_within", "entry_between", 
                  "funding_within", "funding_between",
...
fig.save.cairo(fig.coefs.h2.zoib, filename = "fig-coefs-h2-bayes-zoib",
               width = 5.5, height = 2.5)
```

#### Complete results in a table

```{r h2-table-zoib, results="asis", warning=FALSE}
caption <- "The effect of anti-NGO legislation on the proportion of OECD overseas development assistance (ODA) committed to contentious purposes in the following year (H~2~), full models. Each cell contains the parameter's posterior median, the 95% credible interval, and the probability that the parameter is greater than one. {#tbl:h2-coefs-zoib}"
...
cat(table.h2.zoib)
cat(table.h2.zoib, file = here("Output", "tbl-h2-coefs-bayes-zoib.md"))
```


## H3: Zero-one-inflated beta

```{r h3-models-bayes-domestic-zoib, warning=FALSE, message=FALSE, cache=TRUE}
# Get scaling factors
...
mods.h3.dom.next_year.bayes.zoib <- mods.h3.dom.next_year.raw.zoib %>%
  ...
  mutate(glance = model %>% map(glance),
         tidy = model %>% map(tidy)) %>% 
  bind_cols(select(scales.h3.dom.next_year.zoib, -m))
```

```{r h3-models-bayes-foreign-zoib, warning=FALSE, message=FALSE, cache=TRUE}
# Get scaling factors
...
mods.h3.foreign.next_year.bayes.zoib <- mods.h3.foreign.next_year.raw.zoib %>%
  ...
  mutate(glance = model %>% map(glance),
         tidy = model %>% map(tidy)) %>% 
  bind_cols(select(scales.h3.foreign.next_year.zoib, -m))
```
...
```{r h3-table-domestic-zoib, results="asis", warning=FALSE}
caption <- "The effect of anti-NGO legislation on the proportion of US aid channeled through *domestic* NGOs in the following year (H~3~), full models. Each cell contains the parameter's posterior median, the 95% credible interval, and the probability that the parameter is greater than one {#tbl:h3-domestic-coefs-zoib}"
note <- c("Zero-one-inflated beta models. Odds ratios reported.")

table.h3.domestic.zoib <- bayesgazer(mods.h3.dom.next_year.bayes.zoib,
                                     caption = caption, note = note, exponentiate = TRUE)

cat(table.h3.domestic.zoib)
cat(table.h3.domestic.zoib, file = here("Output", "tbl-h3-domestic-coefs-bayes-zoib.md"))
```

#### Proportion to foreign NGOs

```{r h3-table-foreign-zoib, results="asis", warning=FALSE}
caption <- "The effect of anti-NGO legislation on the proportion of US aid channeled through *US-based and international* NGOs in the following year (H~3~), full models. Each cell contains the parameter's posterior median, the 95% credible interval, and the probability that the parameter is greater than one {#tbl:h3-foreign-coefs-zoib}"
note <- c("Zero-one-inflated beta models. Odds ratios reported.")
...
cat(table.h3.domestic.zoib)
cat(table.h3.domestic.zoib, file = here("Output", "tbl-h3-foreign-coefs-bayes-zoib.md"))
```

#### Plot of proportion to domestic and foreign NGOs {.active}

```{r plot-h3-coefs-both-zoib, warning=FALSE, message=FALSE, fig.width=5.5, fig.height=3}
vars.to.plot <- c("barriers.total_within", "barriers.total_between",
                  "advocacy_within", "advocacy_between", 
                  "entry_within", "entry_between", 
                  "funding_within", "funding_between",
...
fig.save.cairo(fig.coefs.h3.zoib, filename = "fig-coefs-h3-bayes-zoib",
               width = 5.5, height = 3)
```
