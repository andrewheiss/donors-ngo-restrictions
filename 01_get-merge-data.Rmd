...
purpose.codes.contentiousness <- read_csv(here("Data", "data_clean",
...
ever.dac.eligible <- read_csv(here("Data", "data_clean", 
...
dcjw.questions.raw <- read_csv(here("Data", "data_clean", "dcjw_questions.csv"))
...
- `entry` (Q2b, Q2c, Q2d; 3 points maximum, actual max = `r max(dcjw.full$entry)` points maximum): barriers to entry
    - Q2c is reversed, so *not* being allowed to appeal registration status earns 1 point.
    - Q2a is omitted because it's benign
- `funding` (Q3b, Q3c, Q3d, Q3e, Q3f; 5 points maximum, actual max = `r max(dcjw.full$funding)`): barriers to funding
    - Q3a is omitted because it's benign
    - Scores that range between 0–2 are rescaled to 0–1 (so 1 becomes 0.5)
- `advocacy` (Q4a, Q4c; 2 points maximum, actual max = `r max(dcjw.full$advocacy)`): barriers to advocacy
    - Q4b is omitted because it's not a law
    - Scores that range between 0–2 are rescaled to 0–1 (so 1 becomes 0.5)
- `barriers.total` (10 points maximum, actual max = `r max(dcjw.full$barriers.total)`): sum of all three indexes

These indexes are also standardized by dividing by the maximum, yielding the following variables:

- `entry_std`: 1 point maximum, actual max = `r max(dcjw.full$entry_std)`
- `funding_std`: 1 point maximum, actual max = `r max(dcjw.full$funding_std)`
- `advocacy_std`: 1 point maximum, actual max = `r max(dcjw.full$advocacy_std)`
- `barriers.total_std`: 3 points maximum, actual max = `r max(dcjw.full$barriers.total_std)`

```{r dcjw-glimpse}
dcjw.full %>% glimpse()
```

### Civil society regulatory environment {.tabset .tabset-fade .tabset-pills}
...
There's no direct link for V-Dem data, since it's behind a contact information form. So, download the [8.0 version of the "Country-Year: V-Dem + other" dataset](https://www.v-dem.net/en/data/data-version-8/) and place it in `Data/data_raw/V-Dem/v8`
...
```{r load-vdem, warning=FALSE, message=FALSE, cache=TRUE}
vdem.raw <- read_csv(here("Data", "data_raw", "V-Dem", "v8",
                          "V-Dem-CY+Others-v8.csv"))

testthat::expect_equal(nrow(vdem.raw), 26537)
testthat::expect_equal(ncol(vdem.raw), 4641)
```

```{r clean-vdem, cache=TRUE}
...
vdem.cso.min %>% head(50) %>% datatable(extensions = "Responsive")
```
...
### World Bank and UN development indicators {.tabset .tabset-fade .tabset-pills}
...
# UN country codes
un.codes <- read_csv(here("Data", "data_clean", "un_codes.csv")) %>%
  select(un_code, iso3)
...
### Former colonial status {.tabset .tabset-fade .tabset-pills}
...
```{r colonial-status-table}
col.clean %>% datatable(extensions = "Responsive")
```

#### Summary of clean data

```{r colonial-status-glimpse}
col.clean %>% glimpse()
```
...
### Natural disasters {.tabset .tabset-fade .tabset-pills}
...
```{r load-disasters, message=FALSE, cache=TRUE}
...
```{r log-lead-lag-everything, cache=TRUE}
...
```{r plot-missing-imputed, fig.width=8, fig.height=5, cache=TRUE}
...
```{r save-final-data, echo=FALSE, cache=TRUE}
saveRDS(filter(country.aid.data, m == "original"),
        file = here("Data", "data_clean", "df_country_aid_no_imputation.rds"))

saveRDS(filter(country.aid.data, m != "original"),
        file = here("Data", "data_clean", "df_country_aid_imputation.rds"))

saveRDS(filter(country.aid.data.m10, m != "original"),
        file = here("Data", "data_clean", "df_country_aid_imputation_m10.rds"))

saveRDS(donor.level.data,
        file = here("Data", "data_clean", "df_donor.rds"))

saveRDS(donor.level.data.usaid,
        file = here("Data", "data_clean", "df_donor_usaid.rds"))
```
