...
h2.ivs <- c(get.terms(ivs.h2.barriers.total), 
            get.terms(ivs.h2.barriers.new), 
            get.terms(ivs.h2.type.total), 
            get.terms(ivs.h2.type.new), 
...
mod.h2.barriers.new.bayes <- function(df, outcome) {
  cat("#####\n")
  cat("Running model for mod.h2.barriers.new.bayes \n")
  cat("#####\n")
  
  stan_glmer(build.formula(outcome, ivs.h2.barriers.new),
             data = df, family = gaussian(),
             prior = normal(), prior_intercept = normal(),
             chains = CHAINS, iter = ITER, warmup = WARMUP, seed = BAYES.SEED)
}
...
mod.h2.type.new.bayes <- function(df, outcome) { 
  cat("#####\n")
  cat("Running model for mod.h2.type.new.bayes \n")
  cat("#####\n")
  
  stan_glmer(build.formula(outcome, ivs.h2.type.new),
             data = df, family = gaussian(),
             prior = normal(), prior_intercept = normal(),
             chains = CHAINS, iter = ITER, warmup = WARMUP, seed = BAYES.SEED)
}
