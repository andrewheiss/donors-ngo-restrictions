...
stargazer.fake <- function(df, term.labels) {
  top.coefs <- df %>%
    unnest(tidy.melded) %>%
  ...
  pandoc.table(nice.top.bottom, keep.line.breaks=TRUE)
}
```
...
mods.robust.check.m.next_year.melded.all <- bind_rows(
  ...
  mutate(model.name = paste(model.name, m, sep="_"))
```

```{r tbl-show-imputations, results="asis"}
mods.robust.check.m.terms <- c("barriers.total_between", "barriers.total_within", "barriers.total_new", "advocacy_between", "advocacy_within", "entry_between", "entry_within", "funding_between", "funding_within", "advocacy_new", "entry_new", "funding_new", "csre_between", "csre_within", "polity_between", "polity_within", "gdp.capita_log_between", "gdp.capita_log_within", "trade.pct.gdp_between", "trade.pct.gdp_within", "corruption_between", "corruption_within", "total.oda_log_between", "total.oda_log_within", "internal.conflict.past.5TRUE", "natural_disaster.occurrence", "(Intercept)")

mods.robust.check.m.model.order <- mods.robust.check.m.next_year.melded.all$model.name

stargazer.fake(mods.robust.check.m.next_year.melded.all, mods.robust.check.m.terms)
```
...
mods.h1.next_year.raw <- df.country.aid.demean.next_year.both %>%
  ...
  mutate(mod.h1.barriers.total = data %>% map(mod.h1.barriers.total,
         ...
         mod.h1.type.total = data %>% map(mod.h1.type.total,
                                          "total.oda_log_next_year"),
         mod.h1.advocacy.total = data %>% map(mod.h1.advocacy.total,
                                          "total.oda_log_next_year"),
         mod.h1.entry.total = data %>% map(mod.h1.entry.total,
                                          "total.oda_log_next_year"),
         mod.h1.funding.total = data %>% map(mod.h1.funding.total,
                                          "total.oda_log_next_year"),
...
h1.terms <- c("barriers.total_between", "barriers.total_within", "barriers.total_new", "advocacy_between", "advocacy_within", "entry_between", "entry_within", "funding_between", "funding_within", "advocacy_new", "entry_new", "funding_new", "csre_between", "csre_within", "polity_between", "polity_within", "gdp.capita_log_between", "gdp.capita_log_within", "trade.pct.gdp_between", "trade.pct.gdp_within", "corruption_between", "corruption_within", "total.oda_log_between", "total.oda_log_within", "internal.conflict.past.5TRUE", "natural_disaster.occurrence", "(Intercept)")

stargazer.fake(mods.h1.next_year.melded, h1.terms)
```

Effect on ODA after 2 years:

```{r h1-melded-table-2, results="asis", warning=FALSE}
stargazer.fake(mods.h1.after_2.melded, h1.terms)
```

Effect on ODA after 5 years:

```{r h1-melded-table-5, results="asis", warning=FALSE}
stargazer.fake(mods.h1.after_5.melded, h1.terms)
```
