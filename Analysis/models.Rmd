...
`TODO`: Consistent colors (PSRI, Honey Pot, or other palette on Adobe Color)
...
$$ln(\frac{\text{contentious aid}}{\text{noncontentious aid}})_{i, t+1} = \text{NGO legislation}_{it} + \text{controls}_{it}$$
...
`TODO`: Convert these logit coefficients into [something more interpretable](https://acrowinghen.com/2014/04/24/interpreting-coefficients-from-a-logit-linear-model-with-a-proportional-dependent-variable/)

```{r h2-melded-table-1, results="asis", warning=FALSE}
stargazer.fake(mods.h2.next_year.melded)
```
