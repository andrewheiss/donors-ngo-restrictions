...
```{r h1-models, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r h1-melded-table, results="asis", warning=FALSE}
...
model.order <- c("mod.h1.barriers.total", "mod.h1.barriers.new", "mod.h1.type.total", "mod.h1.type.new", "mod.h1.csre")

mods.h1.next_year.melded.nice <- mods.h1.next_year.melded %>%
  unnest(tidy.melded) %>%
  ...
  mutate(term = factor(term, levels=h1.terms, ordered=TRUE)) %>%
  arrange(term) %>%
  select_(.dots = c("term", model.order))

ncols <- nrow(mods.h1.next_year.melded)
bottom <- mods.h1.next_year.melded %>%
  select(model.name, n) %>%
  mutate(term = "Observations",
         n = scales::comma(n)) %>%
  spread(model.name, n) %>%
  rbind(c("Country random effects", rep("Yes", ncols))) %>%
  rbind(c("Year random effects", rep("Yes", ncols)))

nice.top.bottom <- bind_rows(mods.h1.next_year.melded.nice, bottom)
