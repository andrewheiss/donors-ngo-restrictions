...
cat(var.summary)
cat(var.summary, file=file.path(PROJHOME, "Output", "tbl-var-summary.md"))
```
...
df.regulation <- df.country %>%
  select(cowcode, year, ngo_register, ngo_register_burden) %>%
  group_by(year) %>%
  summarise(`Registration required` = sum(ngo_register) / n(),
            `Registration burdensome` = sum(ngo_register_burden) / n()) %>%
  gather(type.of.law, value, -year) %>%
  mutate(type.of.law = factor(type.of.law, levels=))
